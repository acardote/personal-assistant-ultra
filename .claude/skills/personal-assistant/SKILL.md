...
- **`active: <slug>` with `age: <h>h`** where age ≤ 4h — load the project's context. Run `tools/project.py resume <slug>` to print `project.md` + manifest + `notes.md` and read it into your working context. Phase 3 routes artefacts under `<content_root>/projects/<slug>/artefacts/<kind>/`. Remember `PA_PROJECT_ID=<slug>` and prepend `export PA_PROJECT_ID=<slug> &&` to subsequent project-relevant Bash calls in this turn.
- **`active: <slug>` with `STALE`** flag (age > 4h) — surface to the user verbatim: *"Previous active project `<slug>` from `<age>h` ago. Run `/personal-assistant project resume <slug>` to continue, or `/personal-assistant project clear` to start fresh."* Proceed in flat-artefact mode until the user disambiguates. Do NOT silently inherit the stale project — that's the F6 bleed the staleness rule is designed to prevent.
...
3. **Write per-type, then lint, then commit + push**:
   - **Artefact**: target depends on whether a project is active (per the activation contract's project check above):
     - **Project-active path** (`PA_PROJECT_ID=<slug>` set): write to `<content_root>/projects/<slug>/artefacts/<kind>/art-<uuid>.<ext>`. Frontmatter MUST include `project_id: <slug>` (per ADR-0003 Amendment 1). Update the project's `notes.md` if the work advances the project (append a dated bullet to `## What's next` describing what came next, what's outstanding). Touch `last_active` on `project.md` (write `tools/project.py` re-exposes this via the helpers used by promote/copy, or rewrite the field by hand surgically — both are fine).
...
- **Phase 3**: propose path `<content_root>/projects/20260507-q3-strategy-doc-a91f/artefacts/draft/art-<uuid>.md`. User approves. Skill writes the file, surgically updates `project.md` to set `last_active: <today>`, optionally appends a dated bullet to `notes.md`'s `## What's next` ("competitive positioning section drafted; needs board review"), runs `tools/lint-provenance.py --require-vault` then `tools/live-commit-push.sh <content_root> "art: draft competitive-positioning in 20260507-q3-strategy-doc-a91f"`, surfaces the SHA.
