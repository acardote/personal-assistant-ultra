...
> **Status**: `v0.10.0` shipped — closed-and-reconciled: concurrent personal-assistant-ultra sessions with isolated vault state per project (#214 closer, parent of #215 investigation / #217 env-var support / #218 helper / #219 docs-and-skill-audit). Lands the `PA_CONTENT_ROOT` env-var routing primitive in `tools/_config.py:load_config()` (precedence over `.assistant.local.json`; honors the same absoluteness + not-inside-method-root validation as the file path; surfaces `config_source: "env"`; suppress the routing breadcrumb via `PA_QUIET=1`) plus the `scripts/pa-session` Python launch helper (verbs: `new`, `resume`, `list`, `close --merge|--keep-branch`, `path`, `doctor`) plus vault-internal worktree layout at `<vault>/.pa-worktrees/<short>/` on branch `project/<short>` (`new` first-run also commits a one-time `/.pa-worktrees/` line to `<vault>/.gitignore` on main). `docs/setup.md` gains a "Working on multiple projects in parallel" section; `SKILL.md` announces the routing primitive; `.assistant.local.json.example` documents the env var; `release-policy.yaml` + `RELEASE.md` register `scripts/pa-session` as a release-scope helper. Backward-compatible: when `PA_CONTENT_ROOT` is unset, behavior matches `v0.9.3` exactly. 23 new acceptance tests (10 `_config` + 13 `pa-session`) and the existing 22 `test_project_acceptance` tests continue to pass. Carry-over from `v0.9.3`'s bundle: parent #183 (#186 throughput measurement pending; slice 4 falsifier-F2 Move-4 gap acknowledged), parent #178 (harvest routine slice 3 branch cleanup + slice 4 ADR + slice 5 live-fire validation pending), parent #173 (kb-process autonomy, slices 2-3 pending), #165 + #166 still under empirical-monitoring windows through 2026-05-26.
> `latest` tracks the most recent immutable release tag — currently `v0.10.0`. See [`RELEASE.md`](RELEASE.md) for the policy.
